...
class SearchContext(object):
    ...
    def __init__(self, search_interface, output_controller, topic):
        ...
        # A list of all of the actions undertaken by the simulated user in chronological order.
        self._actions = []
        ...
        # Sets to True every time a user picks a new subtopic they has never picked before.
        ...
        self._issued_queries = []
        # A list of SERP impressions in chronological order. The length == issued_queries above.
        self._serp_impressions = []

        # Count of SERPs viewed that were attractive enough to view.
        self._attractive_serp_count = 0
        # Count of SERPs viewed that were judged to be unattractive.
        self._unattractive_serp_count = 0

        # The position in the current SERP we are currently looking at (zero-based!)
        self._current_serp_position = 0
        # This counter is used for the current snippet and document.

        # Snippets that have been previously examined for the current query.
        self._snippets_examined = []
        # Documents that have been previously examined for the current query.
        self._documents_examined = []

        # A list of all snippets that have been seen more than once across the search session.
        self._previously_examined_snippets = []
        # A list of all snippets examined throughout the search session.
        self._all_snippets_examined = []
        # A list of all documents examined throughout the search session.
        self._all_documents_examined = []

        # All documents marked relevant throughout the search session.
        self._relevant_documents = []
        # All documents marked irrelevant throughout the search session.
        self._irrelevant_documents = []

        # 0 - no limit on the number issued. Otherwise, the number of queries is capped
        self.query_limit = 0
        # 0 - no revising of relevance judgements, 1- updates the relevance judgement of snippets
        self.relevance_revision = 0
    ...
    def get_last_action(self):
        ...
        If no previous action is present, None is returned
            - this is only true at the start of a simulated search session.
        """
        if self._actions:
    ...
    def set_action(self, action):
        """
        This method is key - depending on the action that is passed to it, the relevant method handling the tidying
            up for that action is called.
        This is the publicly exposed method for doing some action.
        """
        action_mappings = {
            Actions.SUBTOPIC: self._set_subtopic_action,
    ...
    def _set_subtopic_action(self):
        """
        Called when a user made a change to their subtopic. Prepares everything for them.
        """
        pass
    ...
    def add_issued_query(self, query_text: str, page: int = 1, page_len: int = 1000):
        ...
        def create_query_object():
            ...
            Nested method which returns a Query object for the given query string,
             page number and page length attributes from the parent method.
            """
            query_object = Query(query_text)
            query_object.skip = page
            query_object.top = page_len
            query_object.topic = self.topic
            # Actually issue query and get results (from the search interface)
    ...
    def get_last_query(self):
        ...
        # Return the last element in the list (the latest item)
        return self._issued_queries[-1]
    ...
    def get_document_observation_count(self, selected_document):
        ...
        Returns a zero or positive integer representing the number of times the simulated user has
            seen the given document in previous SERPs.
        If the returned value is 0, the document is new to the user, otherwise the document has been
            seen as many times as the returned value.
        """
        occurrences = 0
    ...
    def get_snippet_observation_count(self, selected_snippet):
        """
        Returns a zero or positive integer representing the number of times the simulated user has
            seen the given snippet in previous SERPs.
        If the returned value is 0, the document is new to the user, otherwise the snippet has been seen as many times
    ...
    def get_subtopic(self):
        """Return the subtopic title"""
        return self._last_subtopic
    ...
    def reached_end_of_serp(self):
        """
        Returns True iif the current position of the SERP has exceeded the number of results available on the SERP
            (e.g. EOF).
    ...
    def get_examined_snippets(self):
        ...
        for the CURRENT QUERY. The most recent snippet to be examined is the last document in the list
            - i.e. snippets are listed in chronological order.
    ...
    def get_all_examined_snippets(self):
        """
        Returns a list of Document objects representing all of the snippets examined by the simulated agent
        over the ENTIRE SEARCH SESSION. The most recent snippet to be examined is the last document in the list
        - i.e. snippets are listed in chronological order.
    ...
    def get_examined_documents(self):
        ...
        The most recent document examined is the last document in the list - i.e. examined documents are listed in
        chronological order.
