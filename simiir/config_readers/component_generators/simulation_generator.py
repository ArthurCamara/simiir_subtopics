...
class SimulationComponentGenerator(BaseComponentGenerator):
    ...
    def __init__(self, simulation_id, config_dict):
        ...
        self.search_interface = self._get_object_reference(
            config_details=self._config_dict["searchInterface"], package="search_interfaces"
        )
    ...
    def prettify(self):
        ...
        return_string = "{0}{1}{2}".format(
            return_string,
            "{0}Search Interface: {1}{2}{3}".format(
                " " * self.output.output_indentation * 2,
                self._config_dict["searchInterface"]["@class"],
                os.linesep,
                self._prettify_attributes(self._config_dict["searchInterface"], self.output.output_indentation),
            ),
            os.linesep,
        )
