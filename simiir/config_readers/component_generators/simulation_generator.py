...
class SimulationComponentGenerator(BaseComponentGenerator):
    ...
    def prettify(self):
        ...
        """
        return_string = "{0}Topic: {1}{2}".format(
            " " * self.output.output_indentation * 2, self.topic.id, os.linesep)
        return_string = "{0}{1}{2}".format(return_string, "{0}Search Interface: {1}{2}{3}".format(" " * self.output.output_indentation * 2,
                                           self._config_dict['searchInterface']['@class'], os.linesep, self._prettify_attributes(self._config_dict['searchInterface'], self.output.output_indentation)), os.linesep)
