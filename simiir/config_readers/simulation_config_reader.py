...
class SimulationConfigReader(BaseConfigReader):
    """
    The Simulation Configuration reader - checks for validity in the supplied settings, and creates a
        series of components for use with the simulations.
    ...
    def __iter__(self):
        """
        Turns the Configuration Reader into an iterator; allowing one to iterate through all the different
    ...
    def __next__(self):
        ...
        def get_next_configuration():
            ...
            # prepare the static options (i.e. output and searchInterface)
    ...
    def __calculate_iterations(self):
        """
        Returns a list of the objects which are to be traversed through, making all the different possible combinations
            of simulations to run. (i.e. all users vs all topics)
        """
        iterables = {}
    ...
    def _validate_config(self):
        """
        Validates the contents of the configuration file - under the assumption that it is well formed and
            conforms to the DTD.
