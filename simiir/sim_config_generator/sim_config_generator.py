...
def tidy_dictionary(dict_repr):
    ...
    dict_repr["simulation"]["searchInterface"] = {
        "class": dict_repr["simulation"]["searchInterface"]["@class"],
        "attributes": to_list(dict_repr["simulation"]["searchInterface"], "searchInterface"),
    }

    dict_repr["simulation"]["user"]["queryGenerator"] = to_list(
        dict_repr["simulation"]["user"]["queryGenerator"], "queryGenerator"
    )
    dict_repr["simulation"]["user"]["textClassifiers"]["snippetClassifier"] = to_list(
        dict_repr["simulation"]["user"]["textClassifiers"]["snippetClassifier"],
        "snippetClassifier",
    )
    dict_repr["simulation"]["user"]["textClassifiers"]["documentClassifier"] = to_list(
        dict_repr["simulation"]["user"]["textClassifiers"]["documentClassifier"],
        "documentClassifier",
    )
    dict_repr["simulation"]["user"]["stoppingDecisionMaker"] = to_list(
        dict_repr["simulation"]["user"]["stoppingDecisionMaker"],
        "stoppingDecisionMaker",
    )
    dict_repr["simulation"]["user"]["logger"] = to_list(dict_repr["simulation"]["user"]["logger"], "logger")
    dict_repr["simulation"]["user"]["searchContext"] = to_list(
        dict_repr["simulation"]["user"]["searchContext"], "searchContext"
    )
...
def create_attribute_markup(attribute_dict):
    ...
    attribute_markup = attribute_markup.format(
        attribute_dict["@name"],
        attribute_dict["@type"],
        value,
...
def generate_topics(dict_repr):
    ...
    def generate_topic_markup(entry):
        ...
        if "@backgroundFilename" in entry:
            ...
            topic_markup = topic_markup.format(
                entry["@id"],
                entry["@filename"],
                entry["@qrelsFilename"],
            ...
            topic_markup = topic_markup.format(entry["@id"], entry["@filename"], entry["@qrelsFilename"])
...
def generate_markup(dict_repr, permutations, gen_filename):
    ...
    for iteration in permutations:
            ...
            if "attribute" in component:
                ...
                if type(component_attributes) == dict:
                    ...
                    for attribute in component_attributes:
                        ...
                        user_markup_components[component_type]["attributes"] = "{0}{1}".format(
                            user_markup_components[component_type]["attributes"],
                            create_attribute_markup(attribute),
                        )
        ...
        user_markup = user_markup.format(
            ...
            if user_markup_components["searchContext"]["attributes"] is not None
            else "",
        )
