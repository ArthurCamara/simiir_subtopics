...
class ReverseGreedySubtopicPicker(BaseSubtopicPicker):
    """Will start at the end and go UP on the list of subtopics."""

    def __init__(self, search_context, seed=None, limit: float = 10.0):
        super(ReverseGreedySubtopicPicker, self).__init__(search_context, limit)

    def pick(self):
        # Check if it's done with current subtopic
        current_subtopic = self._search_context.get_subtopic()
        if current_subtopic is None:
            return self._search_context.topic.subtopics[-1]
