...
class FixedCostLogger(BaseLogger):
    ...
    Costs can be defined in the constructor - and time_limit is the maximum amount of time a search session
        can be carried out for.
    A session ends when the accumulated costs reaches (or exceeds) the specified time limit. If a user initiates an
        activity which pushes them over the limit, they can complete that action - but nothing more.
    """
    ...
    def __init__(
        ...
        self._last_query_time = (
            0  # The last time a query was issued, from the start of the session, measured in seconds.
        )
        self._last_marked_time = (
            0  # The last time a document was marked, from the start of the session, measured in seconds.
        )
        self._last_relevant_snippet_time = (
            0  # The last time a snippet was considered relevant, from the start of the session (seconds).
        )
    ...
    def _report(self, action, **kwargs):
        ...
        Includes additional details in the message such as the total elapsed time, and maximum time available
            to the user after the action has been processed.
        """
        log_entry_mapper = {
        ...
        self._output_controller.log(
            "{0}{1} {2} {3}".format(base, self._time_limit, self._total_time, log_entry_mapper[action])
        )
