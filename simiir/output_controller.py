...
class OutputController(object):
    ...
    def display_config(self):
        ...
        print(f"SIMULATION '{simulation_base_id}'")

        print(f"{' '*self.output_indentation}Simulation Configuration:")
        print(self.__simulation_configuration.prettify())

        print(f"{' '*self.output_indentation}User Configuration ({self.__simulation_configuration.user.id}):")
    ...
    def display_report(self):
        ...
        print()
        print()
        print(f"{' ' * self.output_indentation}Results Summary:")
        print(f"{search_context_summary}{os.linesep}")
    ...
    def __save_simulation_config(self):
        ...
        if self.__save_config_log_flag:
            ...
            log_file.write(os.linesep)
            log_file.write(
                "{0}User Configuration ({1}):".format(
                    " " * self.output_indentation, self.__simulation_configuration.user.id
                )
            )
            ...
            with open(relevance_judgments_filename, "w") as judgments_file:
                for document in search_context.get_relevant_documents():
                    ...
                    judgments_file.write(
                        "{0} Q0 {1} {2} {3} Exp{4}".format(topic.id, document.doc_id, rank, rank, os.linesep)
                    )
