...
class BaseSERPImpression(object):
    ...
    def __init__(self, search_context, qrel_file=None, host=None, port=None):
        ...
        if qrel_file:
            self._qrel_data_handler = get_data_handler(
                filename=qrel_file, host=host, port=port, key_prefix="serpimpressions"
            )
    ...
    def _get_patch_judgements(self):
        ...
        for i in range(0, goto_depth):
            snippet = Document(
                results_list[i].whooshid, results_list[i].title, results_list[i].summary, results_list[i].docid
            )
